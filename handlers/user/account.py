...
async def account_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_vip and subscription_end:
        ...
        if remaining.total_seconds() > 0:
            ...
            if lang == "ar":
                ...
                if days > 0:
                    remaining_text = f"{days} day, {hours} hour"
                elif hours > 0:
                    remaining_text = f"{hours} hour, {minutes} minute"
                else:
                    remaining_text = f"{minutes} minute"
                expiry_date = subscription_end.strftime("%Y-%m-%d %H:%M")
                expiry_status = "✅"
        else:
            if lang == "ar":
    ...
    if lang == "ar":
        ...
        account_text = (
            f"🧑 **Your Profile**\n\n"
            f"🆔 **ID:** `{user_id}`\n"
            f"💎 **Status:** {'🔥 VIP' if is_vip else '🆓 Free'}\n"
            f"📊 **Downloads Today:** {daily_downloads}/{5 if not is_vip else '∞'} 📈\n"
        )
        
        # عرض رصيد النقاط
        if no_logo_credits > 0:
            account_text += f"🎨 **No-Logo Balance:** {no_logo_credits} videos\n"
        
        if is_vip:
            account_text += f"⏱️ **Remaining:** {remaining_text} ⚡\n\n"
        else:
            account_text += f"⏱️ **Remaining:** {remaining_text}\n\n"
        
        account_text += f"{'👑 **VIP Subscriber** 👑' if is_vip else '🆓 Free User'}\n\n"
        
        if is_vip and subscription_end and expiry_status == "✅":
            account_text += (
                f"📦 **Subscription Details:**\n\n"
                f"✅ Downloads: **Unlimited** ∞\n"
                f"✅ Video duration: **Unlimited** ⏰\n"
                f"✅ No watermark 🎨\n"
                f"✅ High quality 📺\n"
            ...
            if no_logo_credits > 0:
                account_text += f"🎨 **Additional no-logo balance:** {no_logo_credits} videos"
        else:
            account_text += (
                f"💡 **Subscribe to VIP for:**\n\n"
                f"✅ **Unlimited** downloads ∞\n"
                f"✅ No waiting ⚡\n"
                f"✅ **4K/8K** quality 🎬\n"
                f"✅ **No watermark** 🎨\n"
                f"✅ Fast technical support 💬\n"
                f"✅ Server priority 🚀\n\n"
            )
            if no_logo_credits > 0:
                account_text += f"🎁 **No-logo balance:** {no_logo_credits} videos\n\n"
            account_text += f"💰 VIP plan starts at **$3/month**\n\n📩 To subscribe: **Contact me on Instagram @7kmmy**\n📲 Or send me a message!"
